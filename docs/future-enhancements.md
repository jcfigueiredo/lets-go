...
## Enhancement F — `make coverage` self-sufficiency

**Why later:** `make coverage` doesn't currently depend on `migrate-test`,
unlike `make test`. From a cold state (`make down && make up`), `make
coverage` would fail because `lab_test` lacks the `alembic_version` table.
Users habitually run `make test` before `make coverage`, so this hasn't
bitten anyone — but adding `coverage: migrate-test` to the Makefile would
